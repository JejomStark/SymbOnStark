...
# Import Kitti interface
from IKITTIS import IKITTIS
...
# set kitti address
@storage_var
func kittis_address_storage () -> (address: felt):
end
...
func set_kittis_address{
    ...
    kittis_address_storage.write(address)
    return()
end

# # Function view to get a owner's symbiote based on its address
# func get_kittis_address{
#         syscall_ptr : felt*, 
#         pedersen_ptr : HashBuiltin*,
#         range_check_ptr
#     }() -> (address: felt):    
#     let (address) = kittis_address_storage.read()
#     return (address)
# end
# Private function to generate a random DNA
@external
func get_kitti_dna{
    ...
    let (address) = kittis_address_storage.read()
    # To call an interface we need to push the contract address before
    let (dna) = IKITTIS.get_kitti(contract_address=address, kittiId=kittiId)
    return (dna)
end
